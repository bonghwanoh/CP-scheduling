...
/* TCP fastopen related information */
	...
	u64	average_rtt;		
	u64	average_rtt_count;	/* sample number for average_RTT	*/ 
	u64	number_add_retrans;	/* the number of opportunistic retransmission	*/ 

	u32	ofo_count;	/* the number of out of ordered packets 	*/ 
	u64	last_ofo_seq;	/* the seq of last out of ordered packets 	*/ 
	u32 	PF_ACK;		// for receiver
	u32 	pre_rcv_nxt;		// for receiver
	u32	seq_PFV;

	u32 	rcv_PF_ACK;
	u32 	data_seq_PF_ACK;
	u32 	stable_path_index;
	u32 	MAX_outstanding_pkts; // calculate maximum outstanding pkts for BBP
	u32 	scheduling_prevention; // 

	u32 	rcv_PFV;
	u32 	rcv_PFV_data_seq;

	u32	prevent_by_FP;
	u32	prevention_time_by_FP;

	u32	blocking_count;
	u32	blocking_seq;
	u32	blocking_RTT;

	u32	pre_arrival_time; 
	u32	pre_arrival_total;
	u32	pre_arrival_count; 
 	u32	pre_arrival_max; 
	ktime_t kstart, kend;
	u32 	kcount;
	u64 	ktotal;
	...
	ktime_t kstart1, kend1;
	u32 	kcount1;
	u64 	ktotal1;
	u64 	kmax1;
	u32 	kabnormal_count1;
	u32 	kinit;
	u32 	prob_count;
	u32 	total_T_guard;
	u32	blocking_for_delay;
	u32	initial_CP;
	// added end.
...
#define CP_MAX_PATH	8
struct CP_subflow_info {

	u64 RTT_subflow[CP_MAX_PATH];
	u64 RTT_subflow_index[CP_MAX_PATH];
	u64 CWND_subflow[CP_MAX_PATH];
	u64 outstanding_subflow[CP_MAX_PATH];
	u64 subflow_ssthresh[CP_MAX_PATH];
	int subflow_mss[CP_MAX_PATH];
